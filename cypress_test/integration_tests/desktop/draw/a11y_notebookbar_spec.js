...
describe(['tagdesktop'], 'Accessibility Draw Notebookbar Tests', { testIsolation: false }, function () {
	...
	it('Notebookbar tab: Table (context)', function () {
		...
		// Table Design shares the Table context where the notebookbar has that tab.
		const tableDesign = findTab('TableDesign');
		if (tableDesign) selectAndValidateTab(tableDesign);
