...
describe(['tagdesktop'], 'Tooltip', { testIsolation: false }, function () {
	...
	before(function () {
		...
		cy.cGet('.ui-dialog #add-button[data-cooltip]').should('be.visible');
	});
	...
	function widget(id) {
		return win.document.querySelector('.ui-dialog #' + id + '-button[data-cooltip]');
	}
	...
	function pointerAway() {
		cy.cGet('.ui-dialog #delete-button').realHover();
		cy.then(function () {
	...
	function hover(id) {
		cy.cGet('.ui-dialog #' + id + '-button[data-cooltip]').realHover();
