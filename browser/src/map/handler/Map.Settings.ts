...
window.L.Map.Settings = window.L.Handler.extend({
	...
	askForSentenceChecking: function (): Promise<void> {
		...
		this._sentenceCheckingAsked = new Promise<void>((resolve) => {
			...
			// The answer may never come: an older engine does not know the
			// command. Give up rather than hold the dialog shut.
	...
	showSettingsDialog: async function (target: string): Promise<void> {
		// Normally answered long before anyone opens this, but the request is
		// sent once at load and a dropped or late answer would leave the
		// sentence checker out of the dialog with no way to get it back.
