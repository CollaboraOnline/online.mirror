...
window.L.Control.JSDialogBuilder = window.L.Control.extend({
	...
	_setup: function(options) {
		...
		this._controlHandlers['tabcontrol'] = JSDialog.tabControl;
		...
		this._controlHandlers['chip'] = JSDialog.chip;
	...
	_unoToolButton: function(parentContainer, data, builder, options) {
		...
		if (data.command || data.postmessage === true) {
			...
			if (data.command && (!tooltip || !tooltip.includes('('))) // Add shortcut to tooltip based on command
				tooltip = JSDialog.ShortcutsUtil.getTooltipLabel(tooltip, data.command);
			else if (tooltip)
	...
	executeActionImpl: function(container, data) {
		...
		if (data.action_type === 'rendered_entry' || data.action_type === 'rendered_combobox_entry') {
			const pendingKey = data.control_id + ':' + data.pos;
			if (app.pendingOnDemandRenderRequests.delete(pendingKey) && app.pendingOnDemandRenders > 0)
