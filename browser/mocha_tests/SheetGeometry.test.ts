...
describe('SheetGeometry', function () {
	describe('SheetGeometry public methods tests', function () {
		...
		if (runOnlyForZooms.length) {
			runTestData = testData.filter((testDataForZoom) =>
				runOnlyForZooms.includes(testDataForZoom.zoom),
			);
		}
	...
	function testsForDocZoom(testDataForZoom: TestDataForZoom) {
		var tileSizePx = 256;
		// Important: this mirrors how it is done in CanvasTileLayer.js
		var tileWidthTwips = Math.round(
			(tileSizePx * 15) / zoomToAbsScale(testDataForZoom.zoom),
		);
		var tileHeightTwips = tileWidthTwips;
		...
		if (runOnlyForParts.length) {
			runPartsData = testDataForZoom.partsTestData.filter((partTestData) =>
				runOnlyForParts.includes(partTestData.part),
			);
		}
	...
	function testsForPart(
		partTestData: PartTestData,
		part: number,
		zoom: number,
		sg: cool.SheetGeometry,
	) {
		...
		it('correctness of getTileTwipsAtZoom()', function () {
			var ttwipsAtZoom = sg.getTileTwipsAtZoom(
				partTestData.tileTwipsAtZoom.inputPoint,
				partTestData.tileTwipsAtZoom.zoomScale,
			);
			nodeassert.deepEqual(
				ttwipsAtZoom,
				partTestData.tileTwipsAtZoom.outputPoint,
			);
		});

		it('correctness of getTileTwipsPointFromPrint()', function () {
			var ttwips = sg.getTileTwipsPointFromPrint(
				partTestData.printTwipsToTile.inputPoint,
			);
			nodeassert.deepEqual(
				ttwips,
				partTestData.printTwipsToTile.gridAlignedOutputPoint,
			);
		});
		...
		it('correctness of getPrintTwipsPointFromTile()', function () {
			...
			var ptwips = sg.getPrintTwipsPointFromTile(
				partTestData.printTwipsToTile.outputPoint,
			);
		...
		it('correctness of getTileTwipsSheetAreaFromPrint()', function () {
			var ptwips = sg.getTileTwipsSheetAreaFromPrint(
				partTestData.printTwipsSheetAreatoTile.inputArea,
			);
			nodeassert.deepEqual(
				ptwips,
				partTestData.printTwipsSheetAreatoTile.outputArea,
			);
		});
		...
		it('correctness of getSize()', function () {
			...
			nodeassert.deepEqual(
				sheetSizeCorePixels,
				partTestData.sheetSize.corePixels,
			...
			nodeassert.deepEqual(
				sheetSizeTileTwips,
				partTestData.sheetSize.tileTwips,
			...
			nodeassert.deepEqual(
				sheetSizePrintTwips,
				partTestData.sheetSize.printTwips,
		...
		it('correctness of getCellRect()', function () {
			...
			var cellBounds = sg.getCellRect(
				cellRectData.col,
				cellRectData.row,
				cellRectData.zoomScale,
			);
			nodeassert.deepEqual(
				cellBounds,
				cellRectData.cpixBoundsAtZoom,
				'Incorrect cell bounds at zoom returned',
			);

			var selfZoomScale = zoomToAbsScale(zoom);
			cellBounds = sg.getCellRect(
				cellRectData.col,
				cellRectData.row,
				selfZoomScale,
			);
			nodeassert.deepEqual(
				cellBounds,
				cellRectData.cpixBoundsAtSelfZoom,
				'Incorrect cell bounds at self zoom returned',
			);
		});
		...
		it('correctness of getCellFromPos()', function () {
			...
			nodeassert.deepEqual(
				cell,
				new cool.Point(cellRectData.col, cellRectData.row),
				'Incorrect cell indices at self zoom returned',
			);
		});
		...
		it('correctness of getSnapDocPosX() and getSnapDocPosY()', function () {
			...
			nodeassert.deepEqual(
				cellX,
				cellRectData.cpixBoundsAtSelfZoom.min.x,
				'Incorrect cellX at self zoom returned',
			);

			var cellY = sg.getSnapDocPosY(midPoint.y, 'corepixels');
			nodeassert.deepEqual(
				cellY,
				cellRectData.cpixBoundsAtSelfZoom.min.y,
				'Incorrect cellY at self zoom returned',
			);
		});
	}
}); // root describe
