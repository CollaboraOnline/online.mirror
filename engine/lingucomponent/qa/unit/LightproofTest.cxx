...
void LightproofTest::setUp()
{
    test::BootstrapFixtureBase::setUp();
    // Nothing to register: the checker finds its rule files in the install
    // tree, which the test runs against.
}

uno::Reference<linguistic2::XProofreader> LightproofTest::getProofreader()
{
    return uno::Reference<linguistic2::XProofreader>(
        m_xSFactory->createInstance(u"com.collaboraoffice.lingu.Lightproof"_ustr), uno::UNO_QUERY_THROW);
}
