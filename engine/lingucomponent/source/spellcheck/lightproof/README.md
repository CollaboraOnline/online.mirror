...
## How a rule reaches the reader
    ...
    <pkg>.dlg   the options, their defaults and their English labels
       |
...
The labels take a different road, and not this one: see step 5 of
*Adding a language*.
...
| `tools/lpcompile.py` | rule table to `.lpr`; also emits the options schema and the dialog labels |
...
5. Add the options to the configuration and to the dialog:
       ...
       lpcompile.py --emit-labels dictionaries/<pkg> <pkg>

   Both print a block to paste. Neither runs at build time, and nothing in
   the build reads a `.dlg`. The first block goes under
   `GrammarChecking/SentenceChecking` in
   `officecfg/registry/schema/org/openoffice/Office/Linguistic.xcs`, the
   second into `settingLabels` in
   ...
   xcu, so the names are an interface. The labels are committed for a
   different reason: they have to be translatable. `browser/Makefile.am`
   names `AdminIntegratorSettings.ts` by hand three times, and two of those
   are the `pot:` target and the prerequisites of `l10n-settings.js`, which
   is built by reading the string set out of that source. Both look in
   `$(srcdir)`. A generated file would land in `browser/typescript_js/`
   instead, be skipped by both, and leave the dialog untranslated without
   failing anything.

   The label text therefore lives twice, once in the package's `.dlg` and
   once in the TypeScript. Only the second is what a reader sees, so fix
   both when an English string is wrong.
6. Add the language to `coolkitconfig.xcu.in`.
...
| pt_BR | none | the table is the only copy, and upstream keeps it that way |
