...
constexpr OUStringLiteral IMPLEMENTATION_NAME = u"com.collaboraoffice.lingu.Lightproof";

// The configuration node holding one group of option flags per rule package.
constexpr OUStringLiteral OPTIONS_NODE
    = u"/org.openoffice.Office.Linguistic/GrammarChecking/SentenceChecking";
...
void Lightproof::loadOptions(Package& rPackage)
{
    ...
    if (!m_xConfigNode)
    {
        try
        {
            ...
            const beans::NamedValue aPath(u"nodepath"_ustr,
                                          cpo::uno::Any(OUString(OPTIONS_NODE)));
...
void Lightproof::discoverPackages()
{
    ...
    OUString aDir = OUString::createFromAscii("$BRAND_BASE_DIR/" LIBO_SHARE_FOLDER
                                             "/lightproof");
...
{
    std::map<OUString, std::vector<OUString>>::const_iterator aFound = m_aAnalyses.find(rWord);
    ...
    return m_aAnalyses.emplace(rWord, std::move(aResult)).first->second;
}
...
std::vector<OUString> Lightproof::stem(const lang::Locale& rLocale, const OUString& rWord)
{
    if (rWord.isEmpty())
        return {};
    std::map<OUString, std::vector<OUString>>::const_iterator aFound = m_aStems.find(rWord);
    if (aFound != m_aStems.end())
        return aFound->second;
    return m_aStems
        .emplace(rWord, query(rLocale, u"<?xml?><query type='stem'><word>"_ustr + rWord
                                           + u"</word></query>"_ustr))
        .first->second;
}
...
OUString Lightproof::suggest(const lang::Locale& rLocale, const OUString& rWord)
{
    if (rWord.isEmpty())
        return rWord;
    std::map<OUString, OUString>::const_iterator aFound = m_aSuggestions.find(rWord);
    ...
    return m_aSuggestions.emplace(rWord, aJoined.makeStringAndClear()).first->second;
}
...
OUString onlyMorph(const OUString& rAnalysis)
{
    ...
    if (nAt < 0)
        return rAnalysis;
    const OUString aTail = u"\\1"_ustr + rAnalysis.copy(nAt + 3);
    return aTail.trim();
}
}
...
const Unit* findUnit(const OUString& rName)
{
    for (const Unit& rUnit : UNITS)
    {
        if (rName.equalsAscii(rUnit.pName))
            return &rUnit;
    }
    return nullptr;
}
...
bool convertUnit(double fValue, const OUString& rFrom, const OUString& rTo, double& rResult)
{
                                 ...
                                 bool bLongForm)
{
    ...
    if (!rRemove.isEmpty())
        aNumber = aNumber.replaceAll(rRemove, OUString());
    if (!rDecimal.isEmpty())
...
OUString Lightproof::getImplementationName() { return IMPLEMENTATION_NAME; }
