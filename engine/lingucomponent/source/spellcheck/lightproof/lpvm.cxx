...
OUString translate(const OUString& rString, const OUString& rFrom, const OUString& rTo)
{
    OUStringBuffer aResult(rString.getLength());
    for (sal_Int32 i = 0; i < rString.getLength(); ++i)
    {
        const sal_Int32 nAt = rFrom.indexOf(rString[i]);
        aResult.append(nAt >= 0 && nAt < rTo.getLength() ? rTo[nAt] : rString[i]);
    }
...
Value run(const Context& rContext, const sal_uInt8* pCode)
{
    ...
    for (;;)
    {
        ...
        switch (nOpcode)
        {
            ...
            case OP_PUSH_CONST:
                aStack.push_back(Value::constant(readU32(pAt)));
                pAt += 4;
                break;
            ...
            case OP_RE_SUB:
            {
                const sal_uInt32 nConstant = readU32(pAt);
                pAt += 4;
