...
const char* NS_MCE = "http://schemas.openxmlformats.org/markup-compatibility/2006";
const char* NS_XML = "http://www.w3.org/XML/1998/namespace";
const char* TAG_XML = "xml";
const char* TAG_ALTERNATE_CONTENT = "AlternateContent";
const char* TAG_CHOICE = "Choice";
const char* TAG_FALLBACK = "Fallback";
...
bool checkMCEElement(const XmlNode* element, const std::vector<McScope>& stack,
                     ErrorCapper& errors)
{
    ...
    if (localName != TAG_ALTERNATE_CONTENT)
    {
        if (stack.empty())
        {
            errors.error(element->line, 0,
                         "Invalid root element: " + mcePrefix + ":" + localName);
        }
        else
        {
    ...
    for (const XmlAttribute& attribute : element->attributes)
    {
        ...
        if (isChoice)
        {
            ...
            {
                errors.error(element->line, 0,
                             mcePrefix + ":Choice element shouldn't have a Requires element with the "
                                 + mcePrefix + " prefix");
            }
        }
    }
                      ...
                      ErrorCapper& errors)
{
    ...
    if (vmlStream && stack.empty())
    {
        ...
        {
            errors.error(element->line, 0,
                         "VML streams need to start with <xml>: 8.1 (Part 4)");
        }
        else
        {
