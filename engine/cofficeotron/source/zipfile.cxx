...
std::string toUtf8(const OUString& text)
{
...
bool ZipArchive::open(const std::string& path, std::string& error)
{
    ...
    size_t got = size ? std::fread(impl->data.getArray(), 1, static_cast<size_t>(size), file)
                      : 0;
    std::fclose(file);
    ...
    try
    {
        impl->zip = std::make_unique<ZipFile>(
            impl->mutexHolder, impl->stream,
            css::uno::Reference<css::uno::XComponentContext>(), true /* initialise */,
            false /* force recovery */, ZipFile::Checks::Default);
    }
...
const std::vector<ZipEntryInfo>& ZipArchive::entries() const
{
    return impl->infoList;
}
...
size_t ZipArchive::centralRecordCount() const
{
    return impl->infoList.size();
}

size_t ZipArchive::localHeaderCount() const
{
    return impl->localHeaders;
}
